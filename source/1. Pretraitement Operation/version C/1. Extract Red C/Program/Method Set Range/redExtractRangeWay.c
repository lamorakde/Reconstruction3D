...
int main()
{
    ...
    // This algorithm is just choose one range for each color element: red, green, blue
	for (i = 0; i < dimy; i++)
	{
		for (j = 0; j < dimx; j++)
		{
			if ( (175<ior[i*dimx + j])&&(50<=iov[i*dimx + j])&&(iov[i*dimx + j]<=150)&&(80<=iob[i*dimx + j])&&(iob[i*dimx + j]<= 200) )
			{
				irr[i*dimx + j] = 255;
				irb[i*dimx + j] = 255;
				irv[i*dimx + j] = 255;
